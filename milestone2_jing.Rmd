...
# Step 1: Create Age_Group and filter rows with add-ons purchased
data <- data %>%
  mutate(
    Age_Group = cut(Age, breaks = seq(0, 100, by = 10), 
                    labels = paste(seq(10, 100, by = 10) - 10, seq(10, 100, by = 10) - 1, sep = "-")),
  ...
  filter(Add_On_Purchase == "Yes")  # Only keep rows where add-ons were purchased

# Step 2: Separate multiple add-ons into individual rows, then filter out any remaining empty values after separation
cleaned_data <- data %>%
  separate_rows(Add.ons.Purchased, sep = ",") %>%
  mutate(Add_On = trimws(Add.ons.Purchased)) %>%  # Remove any extra whitespace from Add-On names
  filter(!is.na(Add_On) & Add_On != "")  # Strictly filter out empty values in Add_On

# Step 3: Filter by a specific age group, e.g., "60-69"
filtered_data <- cleaned_data %>% filter(Age_Group == "60-69")

# Step 4: Create the ggplot
ggplot(filtered_data, aes(x = Product.Type, fill = Add_On)) +
  geom_bar(position = "dodge") +
  labs(title = "Add-On Purchases by Product Type for Age Group 60-69", 
       x = "Product Type", y = "Count") +
  scale_fill_manual(values = c("Accessory" = "blue", "Extended Warranty" = "green", "Impulse Item" = "salmon")) +
  theme_minimal() +
  theme(legend.position = "top")
